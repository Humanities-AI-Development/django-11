...
class AppDetail(LoginRequiredMixin,DetailView):
    ...
    def get_context_data(self,**kwargs):
        ...
        context['parent_list']=self.object.comment_set.filter(parent__isnull=True)
        return context
...
def reply_create(request,reply_pk,):
    comment=get_object_or_404(Comment,pk=reply_pk)
    post=comment.target
    ...
    if request.method=='POST':
        ...
        reply.save()
        return redirect('app:app-detail',pk=post.pk)
    ...
    return render(request,'app/reply_create.html',context)
